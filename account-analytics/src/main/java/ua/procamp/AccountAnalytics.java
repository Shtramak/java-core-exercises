...
public class AccountAnalytics {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(groupingBy(acc -> acc.getSex().equals(Sex.MALE), toList()));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(groupingBy(acc -> acc.getEmail().split("@")[1], toList()));
    }
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream()
                .collect(toMap(Account::getId, x -> x));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByIdForAccountsCreatedOn(int year) {
        return accounts.stream()
                ...
                .collect(groupingBy(Account::getEmail, Collectors.mapping(Account::getBalance, reducing(BigDecimal.ZERO, BigDecimal::add))));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream()
                .collect(groupingBy(acc -> acc.getBirthday().getMonth(), mapping(Account::getFirstName, joining(", "))));
    }
